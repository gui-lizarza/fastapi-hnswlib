...
data = np.random.normal(loc=0.0, scale=1.0, size=(1000, 3))
...
async def make_query(vec: List[float]):
    
    tic = time.time()
    labels, distances = graph.knn_query(vec, k=3)
    toc = time.time()
...
async def add_item(vec: List[float]):
    ...
    tic = time.time()
    graph.add_items(vec, item_count)
    toc = time.time()
